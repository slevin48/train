...
estimate greenhouse‑gas emissions.  The results are saved as figures
under the ``figures`` directory.

The datasets used here include:

* ``frequentation-gares.csv`` – full dataset downloaded from the SNCF open
  data portal containing annual passenger counts by station.  The file is
  stored in the repository and read locally.
* A small sample of the ``Régularité mensuelle TGV`` dataset (first 10
  records) embedded as JSON in this script.  The full dataset is
  difficult to download directly via the network from this environment,
  but the sample still illustrates how to work with the data.
* A small sample of the ``Tarifs Intercités`` dataset (first 10 records)
  embedded as JSON.  This contains the minimum and maximum prices for
  selected Intercités routes.
* A small sample of the ``Liste des gares`` dataset providing station
  coordinates (latitude, longitude) used to compute distances.

Emission factors for trains and cars are taken from a Youmatter article
that compares the carbon footprint of different transport modes.  The
article cites a factor of 14 g CO₂ per passenger–kilometre for French
trains and 55 g CO₂ per passenger–kilometre for an average car【679178350990318†L62-L68】.

Note: due to network restrictions in this environment it is not possible
to fetch the entire punctuality and pricing datasets via the API.  The
analysis therefore focuses on the ridership data and uses the small
samples to illustrate the other sections.
"""
